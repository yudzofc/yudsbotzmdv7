    ...
    try {
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          //if(!user.owner || !user.rowner) return conn.reply(m.chat, `Maaf @${m.sender.split`@`[0]} \n*『 Info 』*\nKami Staff YudsBotz ingin melakukan backup data, dan sedikit meningkatkan performa bot.\nJadi, Kami dari Staff YudsBotz memutuskan untuk melakukan *maintenance*, jika ada yang mau ditanyakan, harap hubungi Real Owner, Terima Kasih\n\n_*Hormat Kami, Staff YudsBotz*_`, m, { mentions: [m.sender] })
...
global.dfail = (type, m, conn) => {
  ...
  global.dfails = (jid, buffer, content, footer, button1, row1, button2, row2, quoted) => {
          ...
          4. Gk suka sama YudsBotz? kick aja bodoh

          

          Owner & Police bot berhak banned permanen / sementara dan leave group jika ada yang melanggar! 

          Hormat kami Staff YudsBotz

          `, '©YudsBotz', 'Terima', '.terima', 'Tidak', '.tidak', m)
